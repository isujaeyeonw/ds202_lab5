...
us2 %>% ggplot(aes(x=long,y=lat, fill=MAKE)) + geom_polygon(aes(group=group)) + geom_path(aes(group=group), color="black") + coord_map() + ggtitle("Most Dangerous Vehicle Make by State")
```
...
us4 <- us %>% left_join(summer_fatal2, by=c("group" = "STATE")) 


us4 %>%
ggplot(aes(x = long, y = lat))  + 
  geom_polygon(aes(group = group, fill=fatal_tot)) + ggtitle("Summer Fatalities by State") + coord_map() +  scale_fill_gradient(low = "white", high = "red")
...
us %>% left_join(winter_fatal2, by=c("group" = "STATE")) %>%
  ggplot(aes(x = long, y = lat))  + 
  geom_polygon(aes(group = group, fill=fatal_tot)) + ggtitle("Winter Fatalities by State") + coord_map() +  scale_fill_gradient(low = "white", high = "blue")

```

I have no clue why my states aren't colored appropriately or why some of my data gets lost in a join.... Texas should be very dark, because it has a high number. I can verify that in the data frames going into the map. 
